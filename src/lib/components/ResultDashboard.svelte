...
<div class="space-y-6 py-6">
	...
	<div class="flex flex-col gap-2">
		<h3 class="flex flex-col gap-3 text-base font-semibold sm:flex-row sm:justify-between">
			...
			{#if !recState.isLoading}
				<div class="badge badge-soft text-xs font-thin badge-warning">
					...
					{:else}
						AI unavailable. Showing rule-based recommendations.
					{/if}
				</div>
			{/if}
		</h3>
		...
		<p class="text-xs text-base-content/70">
			Preliminary guidance for review with a qualified structural engineer. These recommendations do
			not establish building safety or code compliance.
		</p>
		{#if recState.isLoading}
			<p class="text-xs text-base-content/70">
				Showing rule-based recommendations while AI is loading.
			</p>
		{/if}
